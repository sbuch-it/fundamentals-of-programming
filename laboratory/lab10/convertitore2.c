...
#define CLASS_LEN 20 //spazio riservato per la classe delle misure
...
typedef struct {     
	...
	char clas[CLASS_LEN];     //stringa come peso, distanza, pressione...
...
main(){
	...
	for(i=0;i<num_units;i++){
		printf("units[%d]:%s\t%s\t%s\t%lf\n",i,units[i].name,units[i].abbrev,units[i].clas,units[i].standard);
	}
	...
	do{
		...
		if( (status=scanf("%lf %s %s",&quantity,old_units,new_units))==3){
			...
			else if(strcmp(units[old_index].clas,units[new_index].clas )!=0) 
...
void load_units(int max_units,unit_t units[], int *unit_sizep){
	...
	i=0;
	while((status=fscanf(input, "%s%s%s%lf", data.name,data.abbrev,data.clas,&data.standard))!=EOF && i<max_units){
		if(status==4) {
			units[i]=data;
			i++;
		}
